...
解释了Scala的类型抽象体系之后，本节主要描述类的合成方式。Scala的基于混入的类合成（mixin class composition）体系是Brach[6]中的面向对象的线性混入合成（linear mixin compostion）和[14、25]中提出的更加对称的混入模块（mixin modules），以及traits[42]这三者的融合。（注：mixin有些人翻译成混合，有些人翻译成混入）我们先看一个例子，如下这个迭代器的抽象描述：
```Scala
trait AbsIterator[T] {
  def hasNext: boolean
  def next: T
